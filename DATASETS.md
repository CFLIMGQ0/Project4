...
### 14.1 胃镜 reportTitle 同质化划分结果（基于当前同质化计算）

根据你当前给出的同质化计算结果，胃镜相关 `reportTitle` 可按以下五类划分：
...
5. **其他**
   - 胃镜下静脉曲张手术报告（不与所有类别同质）
   - 超声胃镜下手术报告（与手术胃镜和超声胃镜同质）

备注：
- “其他”中的两类用于保留边界样本与交叉同质样本，后续可按建模任务选择“单独成类”或“按任务归并”。
- 若后续同质性阈值（余弦/FID/MMD）调整，建议同步更新本分组结果。
