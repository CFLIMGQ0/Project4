#!/usr/bin/env python3
"""CT-RATE Fourier后单层MLP五折实验的六卡动态多任务队列。"""
...
OUT = ROOT / "outputs/ct_rate_680/amef_fourier_mlp1_fivefold_v2"
RUNNER = "src/scripts/run_ctrate_680_fourier_mlp1.py"
REMOTE_ROOT = "/new_data/Lim/ct_rate_fourier_mlp1_20260920"
DEVICES = [0, 1, 2, 3, 4]
FOLDS = range(1, 6)
...
def build_queue(path: Path) -> None:
    ...
    for fold in FOLDS:
        ...
        jobs.append({
            "id": f"ct_fourier_mlp1_fold_{fold}",
            "phase": "train",
            "memory_mib": 2500,
    ...
    save_json(path, {
        "description": "CT-RATE 680例：完整ACPE双路径，Fourier映射后的绝对/相对投影均改为单层Linear；五折分类",
...
def main() -> None:
    ...
    parser.add_argument("--queue", type=Path,
                        default=ROOT / "outputs/ct_rate_680/fourier_mlp1_pool/queue.json")
