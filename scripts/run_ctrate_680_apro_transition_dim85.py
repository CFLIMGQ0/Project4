#!/usr/bin/env python3
"""CT-RATE五折：仅将ACPE前后过渡分支的宽度从64改为85。"""
...
MODELS = {MODEL_KEY: "AMEF-MIL (ACPE transition_dim=85)"}
LABELS = core.LABELS
INPUT = core.INPUT
...
def protocol():
    ...
    value["position_ablation"] = (
        "position_variant=apro_full; only the visual-context transition branch changes "
        "512->85, (6*85+1)=511, 511->85->1; Fourier and relative-bias dimensions unchanged"
    )
    value["runner"] = "src/scripts/run_ctrate_680_apro_transition_dim85.py"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=ROOT / "outputs/ct_rate_680/amef_apro_transition_dim85_fivefold",
    )
    ...
    with (args.output_dir / "run.lock").open("a") as lock:
        ...
        if saved.exists() and json.loads(saved.read_text())["protocol_sha256"] != current["protocol_sha256"]:
            raise ValueError("ACPE transition_dim=85协议或源码改变，禁止混用已有结果")
        ...
        for index, device in enumerate(args.devices):
            ...
            processes.append(subprocess.Popen(
                command, cwd=SCRIPTS, env=environment,
                stdout=handle, stderr=subprocess.STDOUT,
            ))
        core.core.save_json(args.output_dir / "run_state.json", {
        ...
        while any(process.poll() is None for process in processes):
            ...
            print(f"CT-RATE ACPE transition_dim=85完成：{completed}/5", flush=True)
            time.sleep(15)
        completed = core.core.aggregate(args, current["protocol_sha256"])
        codes = [process.returncode for process in processes]
        status = "complete" if completed == 5 and not any(codes) else "incomplete"
        core.core.save_json(args.output_dir / "run_state.json", {
            "status": status, "completed_fold_jobs": completed,
            "expected_fold_jobs": 5, "worker_exit_codes": codes,
            "started_unix": started, "finished_unix": time.time(),
            "wall_seconds": time.time() - started, "apro_transition_dim": 85,
        })
