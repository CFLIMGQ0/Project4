...
Upper gastrointestinal endoscopy is a core examination for assessing abnormalities of the esophagus and stomach because it allows direct visualization of mucosal morphology and supports targeted inspection, biopsy, follow-up, and subsequent clinical management. In routine practice, however, a single gastroscopy examination usually contains many images from different anatomical sites, and more than one clinically meaningful finding may be present in the same examination. This study focuses on three examination-level findings that are common in gastroscopy reports and have distinct visual characteristics: esophageal submucosal tumor, esophageal mucosal lesion, and gastritis. These findings differ in anatomical location, lesion layer, and endoscopic appearance. Esophageal submucosal tumor typically appears as a focal protruding lesion beneath the esophageal mucosal surface; esophageal mucosal lesion refers to visible superficial mucosal abnormalities such as congestion, erosion, ulceration, patchy changes, or other suspicious changes; and gastritis reflects inflammatory changes of the gastric mucosa, including congestion, edema, erosion, atrophy, or altered mucosal texture. Reliable examination-level recognition of these findings may help improve the consistency of routine gastroscopy interpretation, identify examinations that require focused review or follow-up, and reduce the risk of overlooking coexisting abnormalities.

\textbf{中文翻译：} 上消化道内镜是评估食管和胃部异常的核心检查方式，因为其能够直接观察黏膜形态，并支持重点观察、活检、随访和后续临床处理。然而，在常规临床实践中，一次胃镜检查通常包含来自不同解剖部位的多张图像，同一检查中也可能同时存在多个具有临床意义的发现。本研究聚焦于胃镜报告中较常见且具有明确视觉特征的三类检查级发现：食管黏膜下肿瘤、食管黏膜病变和胃炎。三者在解剖位置、病变层次和内镜表现上均存在差异。食管黏膜下肿瘤通常表现为位于食管黏膜表面以下的局灶性隆起性病变；食管黏膜病变主要指累及食管表浅黏膜的可见异常，如充血、糜烂、溃疡、斑片状改变或其他可疑改变；胃炎则反映胃黏膜炎症性改变，可表现为充血、水肿、糜烂、萎缩或黏膜纹理改变等。对这些发现进行可靠的检查级识别，有助于提高常规胃镜判读的一致性，筛选需要重点复核或随访的检查，并降低同一次检查中共存异常被遗漏的风险。

Artificial intelligence has shown increasing potential in gastrointestinal endoscopy, including lesion detection, anatomical site classification, and computer-aided interpretation of individual endoscopic images.
Previous studies have reviewed deep learning applications in gastrointestinal endoscopy, developed convolutional neural networks for anatomical classification of esophagogastroduodenoscopy images, and demonstrated the feasibility of gastric cancer detection from endoscopic images.
These studies provide important evidence that computer vision can support image review, improve diagnostic consistency, and assist clinical decision-making.
Nevertheless, most existing systems are designed around single-image recognition or single-task lesion detection, whereas routine gastroscopy is inherently examination based: the diagnostic conclusion is made from a set of images, images vary in quality and anatomical coverage, and different findings may coexist within one procedure. Therefore, examination-level multi-label modeling remains an important but less explored problem in upper gastrointestinal endoscopy.\cite{R1,R2,R3}

\textbf{中文翻译：} 人工智能在胃肠内镜中的应用潜力不断增加，包括病灶检测、解剖部位分类以及单帧内镜图像的计算机辅助判读。既往研究综述了深度学习在胃肠内镜中的应用，构建了用于食管胃十二指肠镜图像解剖部位分类的卷积神经网络，并验证了基于内镜图像进行胃癌检测的可行性。这些研究说明，计算机视觉能够支持图像复核、提高诊断一致性并辅助临床决策。然而，现有系统多数围绕单图像识别或单任务病灶检测展开，而常规胃镜检查本质上是检查级任务：诊断结论来自一组图像，图像质量和解剖覆盖范围存在差异，并且不同发现可能在同一次检查中共存。因此，检查级多标签建模仍是上消化道内镜中重要但研究相对不足的问题。

Developing such a model is challenging for several reasons. First, because endoscopic images and reports contain sensitive clinical information, publicly available gastroscopy datasets that specifically cover esophageal submucosal tumor, esophageal mucosal lesion, and gastritis at the examination level are limited. Public gastrointestinal endoscopy datasets such as Kvasir, HyperKvasir, and Kvasir-Capsule have promoted reproducible endoscopic image analysis and benchmarking, but they were not designed for report-guided examination-level classification of these three coexisting gastroscopy findings.\cite{R4,R5,R6} Second, the abnormal finding may appear in only a small subset of images within a complete examination. This creates a weakly supervised multi-instance learning problem in which image-level lesion locations are unavailable and the model must infer diagnostically informative images from examination-level labels.\cite{R7} Third, the number of images differs across examinations. Sampling too few images may miss key lesion evidence, whereas using too many images may introduce redundant or low-quality frames and increase optimization difficulty. Finally, predicting each label independently may ignore clinically meaningful co-occurrence patterns among findings.

\textbf{中文翻译：} 建立此类模型面临多方面挑战。首先，由于内镜图像和检查报告包含敏感临床信息，目前公开可获取且专门覆盖食管黏膜下肿瘤、食管黏膜病变和胃炎检查级标签的胃镜数据集较为有限。Kvasir、HyperKvasir 和 Kvasir-Capsule 等公开胃肠内镜数据集推动了内镜图像分析的可复现研究和算法基准测试，但它们并不是为本文所研究的三类共存胃镜发现的报告引导检查级分类任务而设计。其次，异常发现可能只出现在一次完整检查中的少数图像内。这使该任务形成弱监督多实例学习问题：模型缺少图像级病灶位置标注，只能根据检查级标签推断具有诊断价值的图像。第三，不同检查包含的图像数量并不一致。采样图像过少可能遗漏关键病灶证据，而使用过多图像又可能引入冗余或低质量图像，并增加模型优化难度。最后，若对各标签进行独立预测，可能会忽略不同发现之间具有临床意义的共现模式。

Multiple instance learning provides a natural framework for this setting because it represents each examination as a bag of images and learns from bag-level supervision. Attention-based and weakly supervised MIL methods have been widely explored in medical image analysis, especially in whole-slide image classification, where representative models such as CLAM, DSMIL, TransMIL, and DTFD-MIL improve bag-level prediction by learning instance importance, modeling relationships among instances, or distilling informative features.\cite{R8,R9,R10,R11} These methods suggest that sparse diagnostic evidence can be extracted from large image sets without dense annotations. For model inspection, Grad-CAM offers a widely used gradient-based visualization method that highlights image regions contributing to predictions, which is useful for clinical review of deep learning models.\cite{R12} More broadly, multiple instance learning has been systematically studied as a weakly supervised learning paradigm for scenarios in which instance-level labels are unavailable but bag-level labels are known.\cite{R13} However, standard MIL methods often learn a single examination representation or a single attention distribution. This design may be insufficient for multi-label gastroscopy, because esophageal submucosal tumor, esophageal mucosal lesion, and gastritis may rely on different images and different visual patterns within the same examination.

\textbf{中文翻译：} 多实例学习为这一场景提供了自然的建模框架，因为它可以将每次检查表示为由多张图像组成的包，并利用包级监督进行学习。基于注意力和弱监督的多实例学习方法已广泛应用于医学图像分析，尤其是在全切片图像分类中，CLAM、DSMIL、TransMIL 和 DTFD-MIL 等代表性模型通过学习实例重要性、建模实例关系或蒸馏有效特征来提升包级预测能力。这些方法说明，在缺少密集标注的情况下，模型仍可从大规模图像集合中提取稀疏诊断证据。在模型检查方面，Grad-CAM 是一种常用的基于梯度的可视化方法，能够突出对预测结果贡献较大的图像区域，因此有助于深度学习模型的临床复核。更广义上，多实例学习已被系统研究为一种弱监督学习范式，适用于实例级标签不可用而包级标签已知的场景。然而，标准多实例学习方法通常学习单一检查表征或单一注意力分布。对于多标签胃镜任务而言，这种设计可能不足，因为食管黏膜下肿瘤、食管黏膜病变和胃炎可能依赖同一次检查中的不同图像和不同视觉模式。

Another limitation of conventional approaches is insufficient modeling of label relationships. In general multi-label image classification, CNN-RNN-based methods have been used to model semantic label dependencies and image-label relevance.\cite{R14} Graph convolutional networks have also been introduced to construct label graphs and propagate information among labels for multi-label recognition.\cite{R15} These studies indicate that label dependency modeling is important when multiple labels may co-occur. However, such methods are usually developed for natural image datasets or fixed image-level inputs, rather than variable-length, report-guided, multi-image gastroscopy examinations. This leaves a methodological gap between existing image-level endoscopic AI, general weakly supervised MIL, and clinically realistic examination-level multi-label gastroscopy classification.

\textbf{中文翻译：} 传统方法的另一局限在于对标签关系建模不足。在一般多标签图像分类中，基于 CNN-RNN 的方法已被用于建模语义标签依赖关系和图像-标签相关性。图卷积网络也被引入用于构建标签图，并在多标签识别中实现标签间信息传播。这些研究表明，当多个标签可能共现时，标签依赖关系建模具有重要意义。然而，此类方法通常面向自然图像数据集或固定的图像级输入，而不是图像数量可变、由报告引导的多图像胃镜检查。因此，现有图像级内镜人工智能、通用弱监督多实例学习方法与临床真实检查级多标签胃镜分类之间仍存在方法学空白。

To address these gaps, this study was conducted in collaboration with Zhongshan Hospital, Fudan University, using a real-world clinical gastroscopy archive that included routine white-light gastroscopy, chromoendoscopy, surgical gastroscopy, endoscopic ultrasound, and related examination records. In addition to endoscopic images, the archive contained structured report information, including report title, patient age and sex, procedure name, endoscopic findings, diagnostic conclusion, biopsy site, Helicobacter pylori status, and procedure-related notes. These fields provide clinically meaningful examination-level information for defining target labels and organizing multi-image examinations, while avoiding exhaustive manual image-level lesion annotation. Based on this clinical data setting, we propose a report-guided label graph multiple instance learning framework for weakly supervised multi-label classification of gastroscopy examinations. The framework treats each examination as a bag of endoscopic images, uses report-derived examination-level labels as supervision, applies label-wise attention pooling to identify sparse finding-specific visual evidence, and incorporates a learnable label graph reasoner to model relationships among coexisting gastroscopy findings.

\textbf{中文翻译：} 为解决上述问题，本研究与复旦大学附属中山医院合作，基于真实世界临床胃镜归档数据开展研究。该数据包括常规白光胃镜、染色胃镜、手术胃镜、超声胃镜及相关检查记录。除内镜图像外，数据还包含结构化报告信息，包括报告标题、患者年龄和性别、操作名称、内镜所见、诊断结论、活检部位、幽门螺旋杆菌状态以及操作过程相关备注等。这些字段为目标标签定义和多图像检查组织提供了具有临床意义的检查级信息，同时避免了对图像级病灶位置进行大规模人工标注的需求。基于这一临床数据基础，本研究提出一种报告引导的标签图多实例学习框架，用于胃镜检查的弱监督多标签分类。该框架将每次检查视为由多张内镜图像组成的包，使用由报告生成的检查级标签作为监督信号，通过标签级注意力池化识别稀疏且与特定发现相关的视觉证据，并引入可学习的标签图推理器建模共存胃镜发现之间的关系。

The main contributions of this study are threefold. First, we formulate the recognition of esophageal submucosal tumor, esophageal mucosal lesion, and gastritis as a report-guided, examination-level, weakly supervised multi-label learning task, which better reflects routine gastroscopy in which multiple findings may coexist. Second, we construct a multi-instance learning framework that can learn from examination-level report labels, handle variable numbers of images across examinations, and use label-wise attention to obtain finding-specific representations. Third, we introduce a learnable label graph reasoner to model inter-label relationships and internally validate the proposed method on a retrospective single-center cohort against baseline and adapted weakly supervised MIL methods.

\textbf{中文翻译：} 本研究的主要贡献包括三个方面。首先，本研究将食管黏膜下肿瘤、食管黏膜病变和胃炎的识别建模为报告引导的检查级弱监督多标签学习任务，更符合常规胃镜检查中多种发现可能共存的实际情况。其次，本研究构建了一种多实例学习框架，可基于报告生成的检查级标签进行学习，处理不同检查图像数量不一致的问题，并通过标签级注意力获得不同发现对应的特异性表征。第三，本研究引入可学习的标签图推理器以建模标签间关系，并在回顾性单中心队列上对所提出方法进行内部验证，同时与基线模型和改造后的弱监督多实例学习方法进行比较。
...
\textbf{中文翻译：} 所有实验均使用表~\ref{T1}所示的相同训练集、验证集和测试集划分。模型选择基于验证集宏平均F1，所有结果均在固定内部测试集上报告。由于本研究是三标签检查级多标签分类任务，且标签之间并不互斥，因此将宏平均F1作为主要评价指标。同时报告微平均F1、宏平均ROC-AUC、宏平均PR-AUC、子集准确率、汉明损失和Cohen's kappa，以反映整体判别能力、严格多标签一致性和预测一致性。为了降低随机波动的影响，并避免仅依据点估计判断模型性能，本文对实验中的主要测试指标计算95\%置信区间（CI）。这些区间用于描述内部测试集估计的不确定性，并支持对不同模型、不同标签和不同消融变体进行更稳健的比较。除非特别说明，所有实验均采用相同的预处理、训练流程、checkpoint选择规则和测试阶段判定阈值。
...
\caption{Overall comparison with baseline and adapted state-of-the-art MIL models on the internal test set. Values are shown as point estimates with 95\% confidence intervals.\label{T2}}
\resizebox{\textwidth}{!}{%
\begin{tabular}{ll*{8}{c}}
\toprule
Group & Model & \shortstack{Macro\\Recall} & \shortstack{Macro\\Precision} & \shortstack{Macro\\F1} & \shortstack{Macro\\ROC-AUC} & \shortstack{Macro\\PR-AUC} & \shortstack{Subset\\accuracy} & \shortstack{Hamming\\loss} & Kappa \\
\midrule
Proposed & Label graph MIL & \shortstack{0.9015\\{[}0.8807, 0.9213{]}} & \shortstack{\textbf{0.8340}\\{[}0.8126, 0.8547{]}} & \shortstack{\textbf{0.8650}\\{[}0.8460, 0.8826{]}} & \shortstack{0.9187\\{[}0.9008, 0.9358{]}} & \shortstack{0.9106\\{[}0.8912, 0.9320{]}} & \shortstack{0.7021\\{[}0.6638, 0.7404{]}} & \shortstack{\textbf{0.1400}\\{[}0.1208, 0.1597{]}} & \shortstack{\textbf{0.7206}\\{[}0.6811, 0.7587{]}} \\
\midrule
Baseline & Attention MIL & \shortstack{0.8895\\{[}0.8683, 0.9110{]}} & \shortstack{0.8170\\{[}0.7946, 0.8391{]}} & \shortstack{0.8504\\{[}0.8308, 0.8692{]}} & \shortstack{0.9179\\{[}0.9001, 0.9353{]}} & \shortstack{0.9164\\{[}0.8982, 0.9350{]}} & \shortstack{0.6672\\{[}0.6289, 0.7073{]}} & \shortstack{0.1556\\{[}0.1353, 0.1760{]}} & \shortstack{0.6894\\{[}0.6490, 0.7299{]}} \\
Baseline & Mean pooling & \shortstack{0.9131\\{[}0.8951, 0.9308{]}} & \shortstack{0.8119\\{[}0.7903, 0.8332{]}} & \shortstack{0.8584\\{[}0.8404, 0.8762{]}} & \shortstack{0.9190\\{[}0.9019, 0.9363{]}} & \shortstack{0.9102\\{[}0.8899, 0.9309{]}} & \shortstack{0.6742\\{[}0.6359, 0.7143{]}} & \shortstack{0.1487\\{[}0.1289, 0.1678{]}} & \shortstack{0.7035\\{[}0.6652, 0.7427{]}} \\
Baseline & Transformer-context MIL & \shortstack{0.9036\\{[}0.8839, 0.9237{]}} & \shortstack{0.8195\\{[}0.7970, 0.8395{]}} & \shortstack{0.8589\\{[}0.8404, 0.8757{]}} & \shortstack{\textbf{0.9253}\\{[}0.9089, 0.9412{]}} & \shortstack{\textbf{0.9181}\\{[}0.8995, 0.9370{]}} & \shortstack{0.6812\\{[}0.6411, 0.7178{]}} & \shortstack{0.1475\\{[}0.1289, 0.1678{]}} & \shortstack{0.7057\\{[}0.6653, 0.7429{]}} \\
Baseline & Top-$k$ MIL & \shortstack{0.9182\\{[}0.8993, 0.9349{]}} & \shortstack{0.8124\\{[}0.7915, 0.8340{]}} & \shortstack{0.8606\\{[}0.8431, 0.8767{]}} & \shortstack{0.9220\\{[}0.9044, 0.9389{]}} & \shortstack{0.9106\\{[}0.8901, 0.9316{]}} & \shortstack{0.6620\\{[}0.6254, 0.7038{]}} & \shortstack{0.1487\\{[}0.1301, 0.1678{]}} & \shortstack{0.7037\\{[}0.6660, 0.7409{]}} \\
Baseline & Max pooling & \shortstack{\textbf{0.9668}\\{[}0.9550, 0.9780{]}} & \shortstack{0.6261\\{[}0.6094, 0.6420{]}} & \shortstack{0.7533\\{[}0.7412, 0.7648{]}} & \shortstack{0.8439\\{[}0.8246, 0.8638{]}} & \shortstack{0.8416\\{[}0.8204, 0.8652{]}} & \shortstack{0.1533\\{[}0.1237, 0.1812{]}} & \shortstack{0.3287\\{[}0.3136, 0.3438{]}} & \shortstack{0.3544\\{[}0.3246, 0.3826{]}} \\
\midrule
Adapted SOTA & TransMIL & \shortstack{0.8928\\{[}0.8712, 0.9119{]}} & \shortstack{0.8309\\{[}0.8096, 0.8542{]}} & \shortstack{0.8606\\{[}0.8411, 0.8800{]}} & \shortstack{0.9247\\{[}0.9076, 0.9407{]}} & \shortstack{0.9179\\{[}0.9011, 0.9366{]}} & \shortstack{\textbf{0.7143}\\{[}0.6794, 0.7526{]}} & \shortstack{0.1429\\{[}0.1220, 0.1632{]}} & \shortstack{0.7147\\{[}0.6740, 0.7564{]}} \\
Adapted SOTA & DSMIL & \shortstack{0.9010\\{[}0.8826, 0.9199{]}} & \shortstack{0.8238\\{[}0.8030, 0.8462{]}} & \shortstack{0.8601\\{[}0.8424, 0.8781{]}} & \shortstack{0.9241\\{[}0.9075, 0.9407{]}} & \shortstack{\textbf{0.9181}\\{[}0.8998, 0.9372{]}} & \shortstack{0.6951\\{[}0.6585, 0.7334{]}} & \shortstack{0.1452\\{[}0.1249, 0.1643{]}} & \shortstack{0.7102\\{[}0.6721, 0.7506{]}} \\
Adapted SOTA & DTFD-MIL & \shortstack{0.9378\\{[}0.9215, 0.9527{]}} & \shortstack{0.7716\\{[}0.7529, 0.7904{]}} & \shortstack{0.8452\\{[}0.8305, 0.8600{]}} & \shortstack{0.9177\\{[}0.9009, 0.9340{]}} & \shortstack{0.9098\\{[}0.8916, 0.9299{]}} & \shortstack{0.5714\\{[}0.5314, 0.6115{]}} & \shortstack{0.1719\\{[}0.1539, 0.1899{]}} & \shortstack{0.6583\\{[}0.6220, 0.6936{]}} \\
Adapted SOTA & CLAM-MB & \shortstack{0.9025\\{[}0.8831, 0.9223{]}} & \shortstack{0.7803\\{[}0.7597, 0.8015{]}} & \shortstack{0.8360\\{[}0.8188, 0.8526{]}} & \shortstack{0.9071\\{[}0.8895, 0.9238{]}} & \shortstack{0.9000\\{[}0.8795, 0.9213{]}} & \shortstack{0.5958\\{[}0.5575, 0.6359{]}} & \shortstack{0.1760\\{[}0.1574, 0.1951{]}} & \shortstack{0.6496\\{[}0.6109, 0.6867{]}} \\
Adapted SOTA & CLAM-SB & \shortstack{0.9219\\{[}0.9043, 0.9387{]}} & \shortstack{0.7550\\{[}0.7349, 0.7769{]}} & \shortstack{0.8284\\{[}0.8122, 0.8445{]}} & \shortstack{0.9070\\{[}0.8889, 0.9247{]}} & \shortstack{0.8907\\{[}0.8682, 0.9152{]}} & \shortstack{0.5453\\{[}0.5070, 0.5871{]}} & \shortstack{0.1911\\{[}0.1719, 0.2096{]}} & \shortstack{0.6203\\{[}0.5838, 0.6589{]}} \\
\bottomrule
\end{tabular}
}
\end{table*}

As shown in Table~\ref{T2}, the proposed Label graph MIL achieved the best macro F1 (0.8650), micro F1 (0.8616), hamming loss (0.1400), and Cohen's kappa (0.7206). The 95\% CI results further support this pattern: the proposed model had the most favorable CI profile for the primary decision-oriented metrics, including macro precision (0.8126--0.8547), macro F1 (0.8460--0.8826), hamming loss (0.1208--0.1597), and kappa (0.6811--0.7587). These intervals indicate that the advantage of the proposed model was not driven only by a single point estimate, but remained stable when test-set uncertainty was considered. The improvement over the strongest non-proposed F1 result was modest, and some CIs overlapped among strong MIL baselines, but the proposed model consistently stayed at the upper end of the comparison for F1-based and agreement-based metrics. Transformer-context MIL and DSMIL reached the highest macro ROC-AUC or macro PR-AUC, and TransMIL achieved the highest subset accuracy, indicating that instance-context modeling and transformer-based MIL remain strong comparators for probability ranking or exact-set prediction. However, the proposed model provided the best overall balance for the main task objective, especially when macro F1, hamming loss, and kappa are considered together.

\textbf{中文翻译：} 如表~\ref{T2}所示，本文Label graph MIL在宏平均F1（0.8650）、微平均F1（0.8616）、汉明损失（0.1400）和Cohen's kappa（0.7206）上取得最佳结果。95\% CI结果进一步支持这一趋势：本文模型在主要判定型指标上具有最有利的置信区间表现，包括宏平均精确率（0.8126--0.8547）、宏平均F1（0.8460--0.8826）、汉明损失（0.1208--0.1597）和kappa（0.6811--0.7587）。这些区间说明，本文模型的优势并非仅来自单一点估计，而是在考虑测试集不确定性后仍保持稳定。相对于非本文模型中的最佳F1结果，本文模型的提升幅度并不夸张，且部分强MIL基线之间的CI存在重叠，但本文模型在F1相关指标和一致性指标上始终处于对比结果的前列。Transformer-context MIL和DSMIL在宏平均ROC-AUC或宏平均PR-AUC上取得最高值，TransMIL在子集准确率上取得最高值，说明实例上下文建模和Transformer类MIL在概率排序或严格标签集合预测方面仍是较强对照。然而，结合宏平均F1、汉明损失和kappa来看，本文模型对本研究的主要任务目标提供了更好的整体平衡。
...
\caption{Label-wise internal test performance of the proposed model. Values are shown as point estimates with 95\% confidence intervals.\label{T3}}
\resizebox{\textwidth}{!}{%
...
Table~\ref{T3} shows that gastritis had the strongest label-wise performance, with an F1 of 0.9194, ROC-AUC of 0.9602, and PR-AUC of 0.9502. The 95\% CIs confirmed that the gastritis label maintained the highest and most stable ranges for the main discrimination and decision metrics, including F1 (0.8938--0.9425), ROC-AUC (0.9422--0.9768), and PR-AUC (0.9243--0.9726). This result suggests that the model can reliably aggregate evidence for gastric inflammatory changes, which are often more spatially distributed across an examination. The two esophageal labels remained clinically meaningful but were more challenging: esophageal submucosal tumor achieved high recall (0.9239; 95\% CI, 0.8904--0.9536) but lower precision, whereas esophageal mucosal lesion had a more balanced recall-precision profile and a higher PR-AUC. These CI ranges show that examination-level weak supervision can capture sparse esophageal evidence, but distinguishing localized esophageal abnormalities remains harder and less stable than identifying gastritis.

\textbf{中文翻译：} 表~\ref{T3}显示，胃炎标签取得最强的分标签表现，其F1为0.9194，ROC-AUC为0.9602，PR-AUC为0.9502。95\% CI进一步表明，胃炎标签在主要判别指标和判定指标上保持最高且较稳定的区间范围，包括F1（0.8938--0.9425）、ROC-AUC（0.9422--0.9768）和PR-AUC（0.9243--0.9726）。这一结果提示模型能够较可靠地聚合胃黏膜炎症相关证据，而此类证据往往在一次检查中分布更广。两个食管相关标签仍具有较好的临床可用表现，但识别难度更高：食管黏膜下肿瘤召回率较高（0.9239；95\% CI，0.8904--0.9536）但精确率相对较低；食管黏膜病变的召回率和精确率更加均衡，并取得较高的PR-AUC。这些CI范围说明，检查级弱监督能够捕获稀疏的食管相关证据，但局灶性食管异常的区分仍比胃炎识别更具挑战性且稳定性略低。
...
\begin{table*}[h]
\scriptsize\sf\centering
\caption{Module-effect ablation of label relation reasoners on the internal test set. Values are shown as point estimates with 95\% confidence intervals.\label{T4}}
\resizebox{\textwidth}{!}{%
\begin{tabular}{ll*{8}{c}}
\toprule
Module & Category & \shortstack{Macro\\Recall} & \shortstack{Macro\\Precision} & \shortstack{Macro\\F1} & \shortstack{Macro\\ROC-AUC} & \shortstack{Macro\\PR-AUC} & \shortstack{Subset\\accuracy} & \shortstack{Hamming\\loss} & Kappa \\
\midrule
...
w/o Label Graph Reasoner & Removal & \shortstack{\textbf{0.9409}\\{[}0.9252, 0.9558{]}} & \shortstack{0.7854\\{[}0.7657, 0.8051{]}} & \shortstack{0.8539\\{[}0.8384, 0.8682{]}} & \shortstack{0.9252\\{[}0.9085, 0.9413{]}} & \shortstack{\textbf{0.9211}\\{[}0.9027, 0.9387{]}} & \shortstack{0.6010\\{[}0.5627, 0.6429{]}} & \shortstack{0.1626\\{[}0.1440, 0.1806{]}} & \shortstack{0.6767\\{[}0.6402, 0.7131{]}} \\
Label Self-Attention Reasoner & Attention/sequence & \shortstack{0.9067\\{[}0.8878, 0.9247{]}} & \shortstack{0.8113\\{[}0.7904, 0.8329{]}} & \shortstack{0.8560\\{[}0.8383, 0.8729{]}} & \shortstack{0.9201\\{[}0.9036, 0.9365{]}} & \shortstack{0.9085\\{[}0.8900, 0.9292{]}} & \shortstack{0.6707\\{[}0.6341, 0.7091{]}} & \shortstack{0.1516\\{[}0.1324, 0.1707{]}} & \shortstack{0.6977\\{[}0.6595, 0.7362{]}} \\
Label Transformer Reasoner & Attention/sequence & \shortstack{0.8901\\{[}0.8705, 0.9105{]}} & \shortstack{0.8234\\{[}0.8024, 0.8452{]}} & \shortstack{0.8547\\{[}0.8372, 0.8734{]}} & \shortstack{0.9183\\{[}0.9006, 0.9347{]}} & \shortstack{0.9090\\{[}0.8889, 0.9284{]}} & \shortstack{0.6864\\{[}0.6498, 0.7247{]}} & \shortstack{0.1504\\{[}0.1301, 0.1690{]}} & \shortstack{0.6997\\{[}0.6622, 0.7402{]}} \\
Dynamic Label GAT Reasoner & Graph variants & \shortstack{0.9247\\{[}0.9073, 0.9416{]}} & \shortstack{0.8124\\{[}0.7920, 0.8332{]}} & \shortstack{0.8636\\{[}0.8467, 0.8800{]}} & \shortstack{\textbf{0.9259}\\{[}0.9094, 0.9424{]}} & \shortstack{0.9202\\{[}0.9016, 0.9386{]}} & \shortstack{0.6655\\{[}0.6254, 0.7056{]}} & \shortstack{0.1458\\{[}0.1266, 0.1649{]}} & \shortstack{0.7095\\{[}0.6717, 0.7477{]}} \\
Cosine Dynamic Graph Reasoner & Graph variants & \shortstack{0.9092\\{[}0.8907, 0.9271{]}} & \shortstack{0.8232\\{[}0.8014, 0.8460{]}} & \shortstack{0.8636\\{[}0.8458, 0.8814{]}} & \shortstack{0.9187\\{[}0.9021, 0.9353{]}} & \shortstack{0.9081\\{[}0.8873, 0.9298{]}} & \shortstack{0.7003\\{[}0.6620, 0.7387{]}} & \shortstack{0.1417\\{[}0.1225, 0.1614{]}} & \shortstack{0.7173\\{[}0.6785, 0.7556{]}} \\
Static Co-occurrence GCN Reasoner & Graph variants & \shortstack{0.9079\\{[}0.8891, 0.9264{]}} & \shortstack{0.8230\\{[}0.8008, 0.8453{]}} & \shortstack{0.8629\\{[}0.8442, 0.8801{]}} & \shortstack{0.9191\\{[}0.9021, 0.9357{]}} & \shortstack{0.9089\\{[}0.8880, 0.9304{]}} & \shortstack{0.7003\\{[}0.6620, 0.7387{]}} & \shortstack{0.1423\\{[}0.1231, 0.1620{]}} & \shortstack{0.7161\\{[}0.6769, 0.7543{]}} \\
Low-Rank Label Graph Reasoner & Graph variants & \shortstack{0.8801\\{[}0.8572, 0.8999{]}} & \shortstack{0.8242\\{[}0.8009, 0.8469{]}} & \shortstack{0.8506\\{[}0.8297, 0.8697{]}} & \shortstack{0.9160\\{[}0.8979, 0.9330{]}} & \shortstack{0.9108\\{[}0.8918, 0.9308{]}} & \shortstack{0.6882\\{[}0.6498, 0.7265{]}} & \shortstack{0.1521\\{[}0.1318, 0.1731{]}} & \shortstack{0.6961\\{[}0.6540, 0.7364{]}} \\
Label MLP-Mixer Reasoner & Feature mixer & \shortstack{0.9006\\{[}0.8822, 0.9195{]}} & \shortstack{0.8270\\{[}0.8069, 0.8485{]}} & \shortstack{0.8620\\{[}0.8435, 0.8802{]}} & \shortstack{0.9187\\{[}0.9025, 0.9350{]}} & \shortstack{0.9073\\{[}0.8932, 0.9311{]}} & \shortstack{\textbf{0.7073}\\{[}0.6707, 0.7456{]}} & \shortstack{0.1429\\{[}0.1225, 0.1620{]}} & \shortstack{0.7148\\{[}0.6760, 0.7552{]}} \\
\bottomrule
\end{tabular}
}
\end{table*}

As shown in Table~\ref{T4}, removing the label graph reasoner decreased macro F1 from 0.8650 to 0.8539 and kappa from 0.7206 to 0.6767, supporting the hypothesis that explicit label relation modeling improves multi-label decision consistency. The 95\% CIs provide additional evidence for this interpretation. The full label graph showed the most favorable CI ranges for macro precision (0.8126--0.8547), macro F1 (0.8460--0.8826), hamming loss (0.1208--0.1597), and kappa (0.6811--0.7587), whereas the no-graph variant had lower macro precision, macro F1, and kappa intervals and a higher hamming-loss interval. Among alternative modules, dynamic GAT achieved the highest macro ROC-AUC, and the no-graph variant achieved the highest macro recall and macro PR-AUC. However, the CI profiles across the primary decision-oriented metrics indicate that the full label graph provided the most balanced and stable ablation result, rather than improving only one isolated metric.

\textbf{中文翻译：} 如表~\ref{T4}所示，去除标签图推理器后，宏平均F1从0.8650下降至0.8539，kappa从0.7206下降至0.6767，支持显式标签关系建模能够提升多标签判定一致性的假设。95\% CI为这一解释提供了进一步证据。完整标签图在宏平均精确率（0.8126--0.8547）、宏平均F1（0.8460--0.8826）、汉明损失（0.1208--0.1597）和kappa（0.6811--0.7587）上呈现最有利的置信区间范围；而无标签图变体的宏平均精确率、宏平均F1和kappa区间较低，汉明损失区间更高。在替代模块中，dynamic GAT取得最高宏平均ROC-AUC，无标签图变体取得最高宏平均召回率和宏平均PR-AUC。然而，从主要判定型指标的CI分布来看，完整标签图提供了最均衡且更稳定的消融结果，而不是仅改善某一个孤立指标。
