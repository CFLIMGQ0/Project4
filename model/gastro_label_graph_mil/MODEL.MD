...
训练参数当前按任务拆分维护：TASK1 使用 `configs/task1/train.yaml` 与 `configs/task1/model.yaml`，TASK2 使用 `configs/task2/train.yaml` 与 `configs/task2/model.yaml`。当前 TASK2 也按三标签任务口径运行，默认基础模型建议使用本模型。
...
- `baselines/task1/gastro_baseline/`：包含 5 个胃镜 baseline，分别为标签注意力 MIL、均值池化、最大响应实例、Transformer 上下文 MIL、Top-k 实例聚合。
- TASK2 当前可直接复用 `baselines/task1/gastro_baseline/` 中按 `num_labels` 参数化的 baseline。
