...
CAPTIONS = {
    ...
    "fig:label_cooccurrence": "四个胃镜数据集的原始与重平衡标签共现矩阵。各列依次展示WLE、染色胃镜、手术胃镜和EUS的分布。上排为原始检查分布，下排为每个数据集中五个独立重平衡训练集汇总后的累积分布；验证集和测试集均未重平衡，也未纳入汇总。每个单元格给出相应数量和百分比。对角线单元格表示各标签的边际阳性频数，非对角线单元格表示两两共阳性频数。百分比以对应原始数据集或汇总训练曝光为基数计算。SMT表示食管黏膜下肿瘤，EML表示食管黏膜病变。",
